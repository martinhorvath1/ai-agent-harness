...
- You MAY read any src/ file a surviving mutant lives in, even ones the public API does not
  re-export. `.dependency-cruiser.cjs` exempts hardening tests from `acceptance-tests-use-public-api`
  for exactly this reason.

Rules:
- NEVER modify src/, tests/unit/, or tests/acceptance/. A hook blocks it. If a survivor turns
  out to be a real bug, do not fix it — report it clearly instead; fixing it is the
  implementer's job, and you grading your own fix would defeat the point of the role.
- Only write in tests/hardening/<slug>/.
...
2. For each survivor not already dismissed in survivors.md, try to kill it first:
   write the smallest test in tests/hardening/<slug>/ that distinguishes mutated from
   unmutated behavior. Prefer one assertion per mutant; note which test kills which mutant id.
3. Re-run mutation testing. Anything still alive gets classified, not ignored:
   ...
   - UNTESTABLE — no seam reaches it (e.g. it's behind a boundary hardening tests still can't
     cross). Name the exact barrier and what would have to change to remove it.
